...
class RestaurantApp(system: ActorSystem) extends Terminal {
  ...
  protected def createRestaurant(): ActorRef = {
    val foodLimit = system.settings.config.getInt("restaurant.food-limit")
    system.actorOf(Restaurant.props(foodLimit), "restaurant")
  }
  ...
  protected def createGuest(count: Int, food: Food, foodLimit: Int): Unit =
    for (_ <- 1 to count)
      restaurant ! Restaurant.CreateGuest(food, foodLimit)
